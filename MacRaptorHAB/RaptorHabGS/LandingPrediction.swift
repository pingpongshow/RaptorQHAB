...
// MARK: - Wind Profile (shared with LandingPredictionsView)
...
class LandingPredictionManager: ObservableObject {
    ...
    func updatePrediction(from telemetry: [TelemetryPoint]) {
        ...
        case .floating:
            // Float prediction - assume eventual burst
        ...
        if let pred = prediction {
            // Update with correct descent rate
    ...
    private func calculatePhaseAndDescentRate(from telemetry: [TelemetryPoint]) -> (FlightPhase, Double) {
        ...
        // Calculate vertical speeds over recent points
    ...
    private func calculateWindFromDrift(telemetry: [TelemetryPoint]) {
        ...
        // During descent, calculate drift from expected vertical descent
        let recent = Array(telemetry.suffix(20))
        
        var driftX: Double = 0  // East-West drift (m)
        var driftY: Double = 0  // North-South drift (m)
        ...
        // Direction wind is going TO (add 180 for direction wind is coming FROM)
    ...
    // MARK: - Prediction from Ascent (with altitude-specific wind)
    ...
    private func predictFromAscent(
        ...
        let timeStep: TimeInterval = 30  // seconds
        
        // Ascent phase - step through altitudes
        ...
        // Descent phase - step through altitudes with varying descent rate
        while currentAlt > targetAlt {
            let wind = getWind(at: currentAlt)
            let driftDir = (wind.direction + 180).truncatingRemainder(dividingBy: 360) * .pi / 180
            
            // Descent rate varies with altitude
    ...
    // MARK: - Prediction from Descent (with altitude-specific wind)
    ...
    private func predictFromDescent(
        ...
        var currentAlt = current.altitude
        var time: TimeInterval = 0
        let timeStep: TimeInterval = 10  // seconds (finer resolution during descent)
        
        // Step through descent with altitude-specific wind
        while currentAlt > targetAlt {
            let wind = getWind(at: currentAlt)
            let driftDir = (wind.direction + 180).truncatingRemainder(dividingBy: 360) * .pi / 180
        ...
        // Confidence based on altitude (lower = more confident)
        let confidence: PredictionConfidence
    ...
    private func predictFromFloat(
        ...
        // Assume eventual burst at current altitude or configured burst altitude
        ...
        // Descent from float/burst altitude
