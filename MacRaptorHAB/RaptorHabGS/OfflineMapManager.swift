...
class OfflineTileOverlay: MKTileOverlay {
    ...
    private let onlineOverlay: MKTileOverlay
    
    init(cache: TileCache) {
        self.cache = cache
        // OpenStreetMap tile server (fallback when online)
        self.onlineOverlay = MKTileOverlay(urlTemplate: "https://tile.openstreetmap.org/{z}/{x}/{y}.png")
    ...
    override func url(forTilePath path: MKTileOverlayPath) -> URL {
        // This won't be used since we override loadTile
...
class TileCache {
    ...
    private let queue = DispatchQueue(label: "com.raptorhabgs.tilecache", qos: .utility)
...
class OfflineMapManager: ObservableObject {
    ...
    private init() {
        let cacheDir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("RaptorHabGS")
    ...
    ///   - minZoom: Minimum zoom level (lower = more zoomed out)
    ///   - maxZoom: Maximum zoom level (higher = more detail)
    ...
    private func performDownload(center: CLLocationCoordinate2D, radiusKm: Double, minZoom: Int, maxZoom: Int) async {
        ...
        for tile in tilesToDownload {
            ...
            var request = URLRequest(url: url)
            request.setValue("RaptorHabGS/1.0 (HAB Tracker)", forHTTPHeaderField: "User-Agent")
