...
//  Minimal SHA-256.
//
//  Used to derive the balloon's Meshtastic node id from its callsign the same
//  way the payload derives it, so beacons are attributed without the operator
//  configuring an id by hand. Kept separate from the manager so the
//  derivation can be verified against the Python implementation directly --
//  a mismatch here means the app silently ignores every balloon beacon.
//

import Foundation

/// Minimal SHA-256, so the node-id derivation matches the payload's without
/// pulling in CryptoKit's Digest types across an actor boundary.
enum SHA256Digest {
